...
import ShinyButton from "@/components/marketing/ShinyButton"; // <-- Novi Premium Button
...
export default function MarketingHeader() {
  ...
  return (
    ...
    >
      <div className="mx-auto w-full max-w-7xl px-6 h-20 flex items-center justify-between">
        ...
        <nav className="flex items-center gap-4">
          {!authenticated ? (
            // GUEST: Log in (Clean Glass Style - Bez skakanja)
            <Link
              href="/signin"
              className="px-5 py-2.5 rounded-full text-sm font-medium text-slate-300 border border-white/10 hover:border-white/30 hover:text-white hover:bg-white/5 transition-all"
            >
              {t("Log in")}
            </Link>
          ) : (
            // LOGGED IN: Dashboard (Premium Shiny Button)
            // Malo manji padding da stane lepo u header
