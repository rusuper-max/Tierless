      ...
      {loading ? (
        ...
        <div className="card p-6 text-sm">
          ...
          <div className="mt-3 flex gap-2">
            <button onClick={createBlank} className="btn btn-outline" style={{ borderColor: "var(--accent)" }}>
              New Page
            </button>
        ...
        // >>> ključ: koristimo wrapper sa klasom .table (dark-aware background/border)
                ...
                <tr key={r.meta.slug}>
                  ...
                  <td>
                    <div className="flex flex-wrap gap-2">
                      <Link href={`/p/${r.meta.slug}`} className="btn text-xs">Public</Link>
                      <Link href={`/editor/${r.meta.slug}`} className="btn text-xs">Edit</Link>
                      <button onClick={() => duplicate(r.meta.slug, r.meta.name)} disabled={busy === r.meta.slug} className="btn text-xs">
                        {busy === r.meta.slug ? "Duplicating…" : "Duplicate"}
                      </button>
                      <button onClick={() => remove(r.meta.slug)} className="btn btn-ghost text-xs">Delete</button>
                    </div>
                  </td>
                </tr>
              ))}
