  ...
  return (
    <div className="relative min-h-screen flex flex-col items-center justify-center px-4 text-white">
      ...
      <div className="relative z-10 mb-8 flex flex-col items-center animate-in fade-in slide-in-from-bottom-4 duration-500">
        ...
        <h1 className="text-3xl font-bold text-center">{t("auth.welcomeBack")}</h1>
        <p className="mt-2 text-slate-400 text-center">{t("auth.enterEmail")}</p>
      </div>
      ...
      <div className="relative z-10 w-full max-w-sm space-y-6">
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="space-y-1">
            <label htmlFor="email" className="text-xs font-semibold uppercase tracking-wide text-slate-500 ml-1">{t("auth.emailAddress")}</label>
            <input
              id="email"
              name="email"
              type="email"
          ...
          >
            ...
            {status === "loading" ? t("auth.sendingLink") : t("auth.sendMagicLink")}
          </button>
