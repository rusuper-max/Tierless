...
import * as calcsStore from "@/lib/calcsStore";        // ← JEDINI mini/full store
import * as trash from "@/lib/data/trash";             // ← trash ostaje isti
...
export async function GET(req: Request, ctx: { params: { slug?: string } }) {
  ...
  try {
    ...
    const miniRow = await calcsStore.get(userId, slug); // ← calcsStore
    if (miniRow) {
...
export async function PUT(req: Request, ctx: { params: { slug?: string } }) {
  ...
  try {
    ...
    if (newName) await calcsStore.updateName(userId, slug, newName); // ← calcsStore
...
export async function DELETE(req: Request, ctx: { params: { slug?: string } }) {
  ...
  try {
    // uzmi mini zapis (iz *istog* store-a koji koristi dashboard) i gurni u trash
    const row = await calcsStore.get(userId, slug);       // ← calcsStore
    if (row) {
      await trash.push(userId, row);
    }

    // očisti full + mini u *istom* store-u
    await fullStore.deleteFull(userId, slug).catch(() => {});
    const removedMini = await calcsStore.remove(userId, slug).catch(() => false); // ← calcsStore
